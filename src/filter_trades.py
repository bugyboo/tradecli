...
def filter_menu(settings: Settings=Settings(), current_prices={}):
    ...
    while True:
        ...
        console.print("[blue]Filter Trades by:[/blue] B[dim]uy[/dim], S[dim]ell[/dim], P[dim]rice[/dim], D[dim]ate[/dim], [dim]P/[/dim]L, A[dim]ll[/dim], [dim]S[/dim]o[dim]rted[/dim] or Enter to skip")
        ...
        if opr_filter == 'b':
            ...
            cursor.execute("SELECT ID, trade_date, symbol, opr, filled_qty, price, cost_value, profit_loss, is_position_open FROM TRADES ORDER BY ID")
        ...
        if all_trades:
            query_table = Table(title="Filtered Trades Sorted by Price (Descending)" if opr_filter != 'o' else "Filtered Trades Sorted by ID")
