    ...
    while True:    
        ...
        if trades:
            ...
            for trade in trades:
                ...
                trades_table.add_row(str(counter), str(trade_date), symbol, f"{opr_text} #{str(ID)}", str(filled_qty), f"${price:,.2f}", f"${cost_value:,.2f}", f"{((filled_qty * price) + settings.get_account().fees_usd) / filled_qty :,.2f}", pl_text, pl_text_sar)
                counter += 1
            ...
            trades_table.add_row("Total", "", "", "", str(total_qty), f"{total_cost_value / total_qty :,.2f}", f"${total_cost_value:,.2f}", "", pl_text_total, pl_text_sar_total)
        ...
        totals_table.add_column(f"Cash {cash_ratio:.2%}", justify="right", style="magenta")
        ...
        totals_table.add_column("Trades", justify="right")
